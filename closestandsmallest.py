...
The two subarrays are sorted in ascending order by their number weights if these weights are different, by
their indexes in the string if they have the same weights.'''
...
def closest(strng):
    if strng:
        ...
        xindex = weights.index(x)
        yindex = weights.index(y)
        if yindex >= xindex:
            del weights[xindex]
            yindex = weights.index(y) + 1
            weights.insert(xindex, x)
        return [[x, xindex, int(numbers[xindex])],[y, yindex, int(numbers[yindex])]]
    return []

print(closest("239382 162 254765 182 485944 468751 49780 108 54"))  # [[9, 1, 162], [9, 7, 108]]
print(closest("79257 160 44641 146 386224 147 313622 117 259947 155 58"))  # [[11, 3, 146], [11, 9, 155]]