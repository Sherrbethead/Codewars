...
letter. For example, the input 'sTreSS' should return 'T'. If a string contains
all repeating characters, it should return an empty string ("") or None -- see
sample tests.
"""
...
def first_non_repeating_letter(string):
    ...
    for i in range(len(str_low)):
        if str_low[i] not in str_low[i + 1:] and str_low[i] not in str_low[:i]:
            return string[i]
    return ''
